...
1. En el proyecto de Vercel andá a **Storage → Marketplace Database Providers
   → Redis (Upstash)** y conectalo (tiene un plan gratuito de sobra para esto).
2. Vercel agrega automáticamente las variables `UPSTASH_REDIS_REST_URL` y
   `UPSTASH_REDIS_REST_TOKEN` al proyecto.
3. Volvé a desplegar (Vercel lo hace solo al detectar la nueva integración) y
   el admin va a guardar ahí en lugar del archivo local. El código ya está
   preparado para usar Redis automáticamente si detecta esas variables
