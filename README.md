...
- **Categorías**: crear/editar/eliminar categorías y subcategorías, marcar
  categorías y subcategorías como "destacadas" (se muestran en negrita).
...
- **Cuenta**: cambiar tu propia contraseña, y (solo el usuario `admin`)
  crear o eliminar usuarios adicionales.

### Usuarios y permisos

El usuario `admin` es el **superusuario**: es el único que puede crear,
eliminar usuarios o restablecer la contraseña de otros, y no puede ser
eliminado por nadie (ni por sí mismo). Los usuarios que él cree pueden
editar todo el contenido de la carta (apariencia, categorías, productos)
exactamente igual que `admin`, pero no tienen acceso a la gestión de
usuarios — solo pueden cambiar su propia contraseña desde la pestaña Cuenta.
...
El usuario por defecto es `admin` y la contraseña `admin123` (cambiala en
`.env.local`, o directamente desde la pestaña **Cuenta** del panel una vez
que entraste). Desde ahí también podés crear usuarios adicionales.
...
2. Entrá a [vercel.com](https://vercel.com) → **Add New Project** → importá
   el repositorio → Deploy. Confirmá que el **Framework Preset** quede en
   **Next.js** (Vercel lo detecta solo normalmente).
3. En **Settings → Environment Variables** agregá `ADMIN_PASSWORD` con la
   contraseña que quieras usar para el usuario `admin` en esa demo.
...
Los usuarios del panel (creados desde la pestaña **Cuenta**) se guardan con
el mismo mecanismo que la carta (Redis o archivo local).
...
- Si el hosting final tiene un sistema de archivos persistente (VPS propio,
  por ejemplo), `data/menu.json` y `data/users.json` funcionan sin cambios.
- Si es otro entorno serverless (como Vercel), lo más simple es mantener la
  ...
  ya tenga contratada (Postgres, MySQL, etc.) — la función expone solo
  `getMenu`/`saveMenu`/`getUsers`/`saveUsers`, así que el resto de la app no
  necesita tocarse.
...
app/
  ...
  api/menu/route.js           → GET/PUT del menú completo (PUT requiere sesión)
  api/admin-auth/route.js     → login (usuario + contraseña)
  api/admin-users/route.js    → alta/baja de usuarios y cambio de contraseña
components/                  → pantallas y piezas de UI reutilizables
lib/
  defaultData.js              → carta de ejemplo inicial
  store.js                    → guarda/lee menú y usuarios (archivo local o Redis)
  users.js                    → carga de usuarios + usuario admin de arranque
  passwords.js                → hash/verificación de contraseñas
  auth.js                     → tokens de sesión firmados (HMAC)
  useCart.js                  → maneja el pedido del cliente (sessionStorage)
  theme.js                    → aplica los colores/tipografías configurados
data/
  menu.json                   → "base de datos" del menú en desarrollo local
  users.json                  → "base de datos" de usuarios en desarrollo local
```
