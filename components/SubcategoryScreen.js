      ...
      <nav className="flex flex-col items-center gap-8">
        {category.subcategories.map((sub) => (
          <button
            ...
            className={`font-heading transition-opacity active:opacity-60 ${sub.featured ? 'text-3xl font-bold' : 'text-2xl font-semibold'}`}
            style={{ color: sub.featured ? 'var(--text)' : 'var(--muted)' }}
          >
            {sub.label}
          </button>
        ))}
