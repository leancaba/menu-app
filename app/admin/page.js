    ...
    <div className="min-h-screen bg-neutral-950 text-white pb-32">
      <header className="sticky top-0 z-10 bg-neutral-950/95 backdrop-blur border-b border-neutral-800 px-6 py-4 flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-neutral-500">Conectado como {currentUser}</p>
        </div>
      ...
      <div className="px-6">
        ...
        {tab === 'cuenta' && <CuentaTab token={token} currentUser={currentUser} />}
      </div>
    </div>
  );
}
...
function CuentaTab({ token, currentUser }) {
  ...
  async function handleDeleteUser(username) {
    ...
    setNotice('');
    try {
      const data = await callUsersApi({ action: 'delete', username });
      setUsers(data.users);
    } catch (e) {
      setError(e.message);
    }
    ...
    <div className="max-w-xl flex flex-col gap-8 pb-10">
      ...
      <section>
        ...
        <p className="text-neutral-400 text-sm mb-4">
          Todos los usuarios tienen las mismas capacidades dentro del administrador.
        </p>
        ...
        <div className="flex flex-col gap-2 mb-5">
          {(users || []).map((u) => (
            <div key={u.id} className="flex items-center justify-between rounded-lg bg-neutral-900 px-4 py-3">
              <span>
                {u.username} {u.username === currentUser && <span className="text-neutral-500 text-xs">(vos)</span>}
              </span>
